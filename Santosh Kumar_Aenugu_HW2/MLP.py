...
from layers.relu import ReluLayer # This is a class for a ReLU layer max(x,0)
from layers.soft_max import SoftMaxLayer #This is a class for the Multilayer perceptron
from layers.linear import LinearLayer # This is a class for a LinearTransform layer which takes an input weight matrix W and computes W x as the forward step
from network.sequential import Sequential
from util.dataset import CifarDataset
from loss.cross_entropy import CrossEntropyLoss # This is a class for a sigmoid layer followed by a cross entropy layer,
...
def MLP():
	...
	for epoch in range(num_epoch):
		...
		timer.begin("epoch")
		for iter, batch in enumerate(data.get_train_batches(mini_batch_size)): #BATCHES ARE FORMED HERE
		...
		print("End of epoch:\ttest objective: {0}\ttrain objective: {1}".format(loss_avg_test,loss_avg_train))
		print("\t\ttest error rate: {0}\ttrain error rate: {1}".format(error_rate_test,error_rate_train))
